...
sub _specified_encoding {
  ...
  if ($encoding and ($str !~ /\e/ or $encoding =~ /^ISO-2022/i)) {
    ...
    if (ref($encoder)) {
      ...
      dbg("charset: the specified charset $encoding detected $detected") if ($str eq '');
    }
  }
...
sub _encode_detect {
  ...
  if ($detected) {
    dbg("charset: Encode::Detect::Detector::detect detected $detected");
...
sub _encode_guess {
  ...
  if (ref($encoder)) {
    ...
    if ($detected) {
      $decoded = $encoder->decode($str);
      dbg("charset: Encode::Guess::guess_encoding detected $detected");
    }
  }
  return ($detected, $decoded);
}
